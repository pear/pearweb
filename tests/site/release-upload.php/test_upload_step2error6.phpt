--TEST--
release-upload.php [upload success, case differs of package name]
--COOKIE--
PEAR_USER=cellog;PEAR_PW=hi
--POST--
upload=1
--UPLOAD--
distfile=test_upload_step2/Archive_tar-1.3.2.tgz
--FILE--
<?php
// setup
...
$mock->addDataQuery("SELECT name FROM packages p WHERE p.package_type = 'pear' AND p.approved = 1 AND  p.name = 'Archive_tar'", array(array('name' => 'Archive_Tar')), array('name'));
$mock->addDataQuery("SELECT p.id FROM packages p WHERE ((p.package_type = 'pear' AND p.approved = 1) OR p.package_type = 'pecl') AND  p.name = 'Archive_tar'", array(array('id' => 1)), array('id'));
...
<!-- START MAIN CONTENT -->
  ...
  <td class="content">
    ...
    <div class="errors">ERRORS:<br />You must correct your package.xml file:<ul><li>Package name in package.xml &quot;Archive_tar&quot; MUST match exactly package name on the website &quot;Archive_Tar&quot;</li>
</ul></div>
