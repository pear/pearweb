...
if (isset($_POST['confirmdetails'])) {
    ...
    if ($request->find($_POST['salt']) && $request->pending()) {
        ...
        if ($errors === true) {
            report_success(
                'Your account has been activated, bugs you have opened and ' .
                'comments you have made are now available to the public for viewing'
            );
...
if (isset($_GET['type']) && $_GET['type'] == 'bug') {
    ...
    echo '<p>Please choose a username for opening future bugs/adding ';
    echo 'comments to existing bugs</p>';
    if (!empty($stripped['salt'])
        && strlen($salt = htmlspecialchars($stripped['salt'])) == 32
    ) {
        $request = new PEAR_Bug_Accountrequest;
    ...
    if (empty($stripped['salt'])
        ...
        if (PEAR::isError($result)) {
            ...
            report_success(
                'Your account has been activated, you can now vote in PEAR ' .
                'elections that are for the general PHP public as well as ' .
                'open bugs in the bug tracker'
            );
        } else {
            report_error(
                'There was a problem activating your account, please contact ' .
