...
*/
	...
	if ($proposal->getStatus() == 'proposal') {
		if (isset($_COOKIE['PEAR_USER'])) {
			...
			$bb->horizHeadRow('<label for="comment_field" accesskey="o">C<span class="accesskey">o</span>mment:</label>', $formArray['elements'][0]['html']);
	...
	if (count($proposal->votes) > 0) {
		...
		foreach ($proposal->votes as $vote) {
			...
			$vote_line .= ' ('.make_link('/user/'.$vote->user_handle, $users[$vote->user_handle]['name']).')';
	...
	if (($proposal->status == 'vote')) {
		...
		if (isset($_COOKIE['PEAR_USER']) && $karma->has($_COOKIE['PEAR_USER'],'pear.dev')) {
			if (!ppVote::hasVoted($dbh, $_COOKIE['PEAR_USER'], $proposal->id) && !$proposal->isOwner($_COOKIE['PEAR_USER'])) {
				...
				$bb->horizHeadRow('<label for="vote_field" accesskey="o">V<span class="accesskey">o</span>te</label>', $formArray['elements'][0]['html']." ".$formArray['elements'][0]['label']);
