...
function reloadMe()
{
    var newLocation = '<?php echo strip_tags($_SERVER['PHP_SELF']); ?>?'
                      + 'cid='
 ...
 */
	...
	$total_packages    = number_format($dbh->getOne('SELECT COUNT(DISTINCT id) FROM packages WHERE package_type="pear" and approved=1'), 0, '.', ',');
