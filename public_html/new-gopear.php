...
if (WEBINSTALLER) {
    setupWebInstaller($php_sapi_name);
}
...
if (WEBINSTALLER) {
    doWebinstallerSetupConfigVars($php_sapi_name, $http_proxy, $www_errors);
} else {
    doCLISetupConfigVars($tty, $local, $http_proxy, $pfc_packages);
}

setupConfigVars($origpwd, $config_vars, $config_desc, $desclen, $descfmt, $first, $last);

$progress = 0;

if (WEBINSTALLER) {
    displayWebinstallerPreamble($www_errors);
} else {
    if (WINDOWS) {
        /*
...
setupTempStuff($ptmp, $prefix);


while (!WEBINSTALLER) {
    if (!doCLIGetConfigVars($config_vars, $descfmt, $config_desc, $desclen, $descfmt, $first, $last,
                           $http_proxy, $tty)) {
        break;
    }
}

postProcessConfigVars($config_vars, $config_desc);
...
setupZlibAndPackages($urltemplate, $have_gzip, $install_pfc, $to_install, $installer_packages,
                     $pfc_packages);
...
function setupWebInstaller(&$php_sapi_name)
{
    /*
     * See bug #23069
     */
...
function setupConfigDesc(&$config_desc)
{
    $config_desc = array(
...
function doWebinstallerSetupConfigVars(&$php_sapi_name, $http_proxy, &$www_errors)
{
    @session_start();
    ...
    */
    ...
    if (!isset($_SESSION['go-pear']) || isset($_GET['restart'])) {
        $_SESSION['go-pear'] = array(
            ...
            'config' => array(
                ...
                'php_bin' => $GLOBALS['php_bin'],
                'php_dir' => '$prefix/PEAR',
                'doc_dir' => $GLOBALS['doc_dir'],
                'data_dir' => $GLOBALS['data_dir'],
                'test_dir' => $GLOBALS['test_dir'],
    ...
    $GLOBALS['install_pfc'] = $_SESSION['go-pear']['install_pfc'];
}

function doCLISetupConfigVars(&$tty, &$local, &$http_proxy, $pfc_packages)
{
...
function setupConfigVars(&$origpwd, &$config_vars, &$config_desc, &$desclen, &$descfmt, &$first,
                         &$last)
{
    $origpwd = getcwd();
...
function displayWebinstallerPreamble($www_errors)
{
...
function setupTempStuff(&$ptmp, $prefix)
{
    tmp_dir($ptmp, $prefix);
    $foo = $ptmp;
...
function doCLIGetConfigVars($config_vars, $descfmt, $config_desc, $desclin, $descfmt, $first, $last,
                            $http_proxy, $tty)
{
    static $php_bin_set = false;
    $disp_php_dir = $GLOBALS['php_dir'];
    foreach ($config_vars as $m => $var2) {
        $disp_php_dir = str_replace('$'.$var2, $GLOBALS[$var2], $disp_php_dir);
    }
    $extra = '';
    if ($disp_php_dir != $GLOBALS['php_dir']) {
        $extra = " = $disp_php_dir";
    }
    print "
...
*** \$php_dir = $GLOBALS[php_dir]$extra
";

    foreach ($config_vars as $n => $var) {
        printf("%2d. $descfmt : %s\n", $n, $config_desc[$var], $GLOBALS[$var]);
    }
...
";
    ...
    if (isset($config_vars[(int)$tmp])) {
        ...
        $current = $GLOBALS[$var];
        if(WIN32GUI){
            $tmp = win32BrowseForFolder("Choose a Folder for $desc [$current] :");
        } else {
            print "$desc [$current] : ";
            $tmp = trim(fgets($tty, 1024));
        }
...
function postProcessConfigVars($config_vars, $config_desc)
{
    global $config_vars, $config_desc;
    foreach ($config_vars as $n => $var) {
        for ($m = 1; $m <= count($config_vars); $m++) {
            $var2 = $config_vars[$m];
            $GLOBALS[$var] = str_replace('$'.$var2, $GLOBALS[$var2], $GLOBALS[$var]);
        }
    }
...
function setupZlibAndPackages(&$urltemplate, &$have_gzip, $install_pfc, &$to_install,
                              $installer_packages, $pfc_packages)
{
    if (!extension_loaded('zlib') && !WEBINSTALLER) {
...
function mergeGoPearBundle(&$local_dir)
{
    $dh = false;
...
function bootStrap($package, $tarball, $cvspath, $file, $local_dir)
{
    print 'Bootstrapping: ' . $package . str_repeat('.', 23 - strlen($package));
    if (in_array(basename($file), $local_dir)) {
        ...
        download_url($url, $file, $http_proxy);
        echo "(remote) ";
    }
}

function finishCLIInstall($bin_dir, $php_dir, $tty)
{
    $sep = WINDOWS ? ';' : ':';
